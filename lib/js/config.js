$.fn.csv2arr = function(callback) {
  ...
  if (files.length != 0) {
    if (typeof FileReader !== "undefined") {
      ...
      showTip("请使用 Chrome 或 Firefox", "warning", "500");
    }
  } else {
    showTip("请导入所有文件", "warning", "500");
    $("#tip").fadeOut(100);
  }
};
...
function goTag(e) {
  ...
  var openUrl = "./annotation_tool.html?urlId=" + urlId;
...
function initTr(item) {
  ...
  if (item.checked) {
    content +=
      "<td><button onclick='handleCancel(this)' class='btn btn-danger' type='button'>取消</button></td>";
  } else if (item.state == 2) {
    content +=
      "<td><button onclick='handleReturn(this)' class='btn btn-danger' type='button'>还原</button></td>";
  } else if (item.state == 1) {
    content +=
      "<td><button onclick='goTag(this)' class='btn btn-info'>检查</button><button onclick='handleDelete(this)' class='btn btn-danger' type='button'>删除</button></td>";
  } else {
    content +=
      "<td><button onclick='goTag(this)' class='btn btn-info'>标注</button><button onclick='handleDelete(this)' class='btn btn-danger' type='button'>删除</button></td>";
  }
  content += "</tr>";
  return content;
}
